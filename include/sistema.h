...
public:
	...
	/**
		 * @brief Função responsavel por armazenar os usuarios logados, 
		 * Por padrão o usuario é logado em nenhum servidor e em nehum CanalTexto.
		 * @param id 
		 */
	void setUsuariosLogados(int id);

	/**
		 * @brief Obtem um objeto Usuario pelo id
		 * 
		 * @param id 
		 * @return Usuario 
		 */
	...
	//TODO Consertar essa função Pois ela não está funcionando corretamente pois um usuario pode ter mais de um servidor
	/**
		 * @brief Obtem um  objeto de Servidor pelo id do dono do servidor
		 * 
		 * @param id 
		 * @return Servidor 
		 */
	Servidor getServidorByDonoID(int id);

	/**
		 * @brief checa se o usuario esta logado noservidor
		 * 
	...
	/**
	 * @brief Checa se o servidor existe no sistema
	 * 
	 * @param nome 
	 * @return true 
	 * @return false 
	 */
	bool servidorExiste(const std::string nome);
};
